...
import { CheckForAboutToExpireSubscriptionsCommand } from "../../Application/CheckForAboutToExpireSubscriptions/CheckForAboutToExpireSubscriptionsCommand";
...
export class CheckForAboutToExpireSubscriptionsController extends BaseController<void> {
  ...
  public async aboutToExpire(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      await this.queryBus.ask(new CheckForAboutToExpireSubscriptionsCommand());
      res.status(200).send({});
    } catch (error) {
      next(error);
    }
