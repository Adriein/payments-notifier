...
import { AdminRegisteredDomainEvent } from "../Domain/AdminRegisteredDomainEvent";
...
export class RegisterAdminHandler implements IHandler<void> {
  ...
  public async handle(command: RegisterAdminCommand): Promise<void> {
    ...
    auth.addEvent(new AdminRegisteredDomainEvent(auth.id(), auth.name(), auth.email(), auth.password()));
